...
% FCR - frequency summation
   \node at (-2.75,0.755) [fonte] () {\textbf{Voltage to be tracked}};
   ...
   \node at ($(Int)+(3.5,0)$) [caixalarga] (coseno) {{$\cos  ( \, \widehat{\theta} \, )$\\$\cos ( \, \widehat{\theta} -2 \pi/3)$\\$\cos ( \, \widehat{\theta} + 2\pi /3)$}}; 
    ...
    \draw [densely dotted, ->] ($(SumOmega)+(0.75,-0.1)$) to
        node [pos=1, anchor=north, fonte] () {\textbf{Estimated value of} $\omega$}
        ++(-1.5,-0.75);

    \draw [densely dotted, ->] ($(Int)+(1.25,-0.1)$) to
        node [pos=1, anchor=north, fonte] () {\textbf{Estimated value of} $\theta$}
        ++(-0.75,-0.75);
