...
\node [block] at (4.5,-1.5) (T3) {$\mathcal{T}_+^T$}; 
...
\node [block] at (4.5,-3) (T6) {$\mathcal{T}_-^T$}; 

\draw [<-, fonte] (T1) to node[pos=1, anchor=east] () {$\matr{m}_{abc}$} ++(-1,0);
\draw [->] (T1) -- node[pos=1, anchor=south east] () {$+$} (S1);
\draw [->] (S1) to (F1);
\draw [->] (F1) -- node[pos=1, anchor=west] () {$\matr{m}_{dq+}$} ++(3,0);
\draw [<-] (T3) -| ++(1,1.5);
\draw [->] (T3) -- (T2);
...
\draw [->] (S2) to (F2);
\draw [->] (F2) -- node[pos=1, anchor=west] () {$\matr{m}_{dq-}$} ++(3,0);
\draw [<-] (T6) -| ++(1,-1.5);
\draw [->] (T6) -- (T5);
